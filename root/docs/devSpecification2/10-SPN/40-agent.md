...
|SPN_AGENT_TRUST_CERTIFICATE_ROOT|SPN HUBのTLSサーバ証明書のルートCA|PEM形式のルートCA証明書のファイルパス|
|BIND_ADDRESS|エージェントが listen するバインドアドレス|`0.0.0.0:8080`|

サービスの URNは(例: `urn:chip-in:service:realm1:hub1:database`)は、クライアント証明書のSubject CNで指定します。
