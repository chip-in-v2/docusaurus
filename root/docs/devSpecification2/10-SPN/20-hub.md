...
SPN Hubは、 SPN エンドポイントからの QUIC での接続を受け付けて、閉じた仮想ネットワークを提供します。

SPN HubとSPN エンドポイントがQUIC接続(SPN セッション)し、
...
SPN エンドポイントには、ProviderエンドポイントとConsumerエンドポイントの二種類があり、前者がサーバーサービスの提供者側で、後者がサーバーサービスの利用者側です。

## SPN セッション

SPN セッションは、SPN Hub と SPN エンドポイントの間の接続です。(QUIC のコネクションと同義ですが、SPNのコネクションとの混同を避けるためにあえて、SPN セッションと呼びます。この他に Chip-in では HTTP のセッションやコネクションも登場しますが、それらとも区別してください。)

QUIC接続の確立によりSPNセッションが確立し、このSPNセッションの中に、ユーザはエンドポイント間のストリーム通信(SPNコネクション)を通して利用します。
...
SPNセッションはセッションの開設時と終了時にログを出力します。ログの項目は以下の通り。
...
SPN Hub は、1つのDNS名で複数稼働させることができます。各SPN Hub は異なるグローバルIPを持ちます。DNSは SPN Hub の1つのFQDNに対して起動中のIPアドレスをすべて返すようにします。

エンドポイント側では、起動時に SPN Hub の FQDN を解決し、得られたグローバルIPアドレスの全てに対してSPNセッションを確立します。
また、定期的に SPN Hub の FQDN を解決し、得られたグローバルIPアドレスの変化を監視します。IPアドレスが削除された場合は、内包するSPNコネクションを中断してSPNセッションを終了し別の SPN Hub に接続し直してSPNコネクションを再開します。

エンドポイント側で複数の SPN セッションがある状態で新しいコネクションが必要となった場合はどれか一つのSPNセッションを選択してSPNコネクションを確立します。
...
ただし、エージェントの場合は、以下のように、4つのストリームを接続することに注意してください。
...
SPNコネクションはコネクションの開設時と終了時にログを出力します。ログの項目は以下の通り。
...
spnhubコマンドは起動時とリロードシグナル受信時に構成情報を[インベントリ](/docs/devSpecification/inventory-openapi) から読み込みます。以下の順に読み込みます。リロードシグナルは SIGUSR1 です。
...
`availabilityManagement` が設定されている場合は、クラスタマネージャによるProviderの自動起動・停止が利用できます。

|項目|説明|
|--|--|
|`ondemandStartOnConsumer`|`true`の場合、対応するConsumerエンドポイントがSPNセッションを確立した時に、Providerを自動起動します。|
|`ondemandStartOnPayload`|`true`の場合、対応するConsumerエンドポイントで最初のエンドユーザの通信が発生した時に、Providerを自動起動します。|
|`idleTimeout`|Providerがアイドル状態（アクティブなSPNコネクションがない状態）になってから自動停止するまでの秒数。`0`を指定すると自動停止は無効になります。|
