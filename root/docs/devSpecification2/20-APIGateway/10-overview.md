...
- 対応するサーバ証明書がないリクエストは、TLS接続が失敗する。 
- http 80 番ポートに受信した場合、 301 で https にリダイレクトする (ただし、本システムに設定されていないホスト名の場合は400エラーとする。)
...
JWTのクレームは、ランダムのデバイスIDを含み、レルムの鍵（レルムの deviceIdSigningKey/deviceIdVerificationKeyプロパティ）で署名/検証します。(クレームの詳細は、[ルーティングアクション](routing-action)を参照)

Cookie はレルムの設定のデバイスコンテキスト保持期間（レルムの deviceContextTimeout プロパティ=*25H2では未実装、定数固定値）に従って有効期限が設定さ、保持期間の 50% の時間を経過してアクセスすると同じデバイスIDで Cookie が再発行されます。
...
これを避けるために API-Gateway では Accept ヘッダを見てクライアントが受け入れ可能なタイプのコンテンツを返す仕組みを提供します。(*25H2では未実装)
...
|:-----------|:---------|:-------------------|:-------------|:----------------|
| www.test.example.com | /shop/ |requireAuthentication  | shop_scope | front.js server |
| www.test.example.com | /shop/api/ | proxy | shop_scope | api server for shop data|
| www.test.example.com | /images/ | proxy | none | static content server |
